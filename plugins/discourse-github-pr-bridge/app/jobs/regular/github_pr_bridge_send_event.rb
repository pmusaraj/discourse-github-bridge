...
module Jobs
  class GithubPrBridgeSendEvent < ::Jobs::Base
    ...
    def execute(args)
      ...
      record_comment_correlation(args[:payload], result)
    end

    private

    def record_comment_correlation(payload, result)
      return if payload[:event_type].to_s != "discourse_post_created"
